...
class VisualEmbedding(nn.Module):
    def __init__(self, config):
        ...
        if config.two_prefix:
            feat_embedding2 = [nn.Linear(feat_dim, config.d_model)]
            if self.config.use_vis_layer_norm:
                feat_embedding2.append(T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon))
...
class VLT5RESNET(T5ForConditionalGeneration):
