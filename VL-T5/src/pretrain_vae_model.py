...
from modeling_t5_vae import VLT5VAE


class VLT5Pretraining(VLT5VAE):
    def __init__(self, config):
        super().__init__(config)

        self.losses = self.config.losses.split(',')
        self.image_emb = nn.Embedding(config.image_size, config.d_model)
    ...
    def train_step(self, batch):
        ...
        codes = self.vae.get_codebook_indices(tensor_img)
        vis_feats = self.image_emb(codes)

        # codes = batch['vis_feats'].to(device)
        # # tensor_img = batch['vis_feats']
        # input_ids = batch['input_ids'].to(device)
        # vis_feats = self.image_emb(codes)
