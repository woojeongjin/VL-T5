...
class JointGPT2Model(GPT2Model):
    ...
    def forward(
        ...
        if position_ids is None:
            position_ids = torch.arange(past_length, input_shape[-1] + V_L + past_length, dtype=torch.long, device=device)
            position_ids = position_ids.unsqueeze(0).view(-1, input_shape[-1] + V_L)
